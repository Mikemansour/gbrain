...
describeE2E('v0.41.6.0 — sync lock recovery scenarios', () => {
  ...
  test('lock-busy error message includes PID + hostname + age + --break-lock hint', async () => {
    ...
    try {
      // This test owns the lock boundary, not embedding credential preflight.
      // Keep the subprocess import-only so a host without the default
      // provider key reaches lock acquisition deterministically.
